...
from college_management_app.models import Attendance, AttendanceReport, Course, CustomUser, FeedbackStaff, LeaveReportStaff, NotificationStaffs, OnlineClassRoom, SessionYearModel, Staffs, StudentResult, Students, Subjects
...
def staff_apply_leave_save(request):
    if request.method != 'POST':
        ...
        staff_obj = Staffs.objects.get(admin=request.user.id)
        try:
            staff_report = LeaveReportStaff(staff_id=staff_obj, leave_date=leave_date, leave_message=leave_msg, leave_status=0)
            staff_report.save()
            messages.success(request, 'Successfully Applied For Leave')
            return HttpResponseRedirect(reverse('staff_apply_leave'))
        except:
            messages.error(request, 'Failed To Apply To Leave')
            return HttpResponseRedirect(reverse('staff_apply_leave'))
...
def staff_feedback_save(request):
    if request.method != 'POST':
        ...
        staff_obj = Staffs.objects.get(admin=request.user.id)
        try:
            staff_feedback = FeedbackStaff(staff_id=staff_obj, feedback=feedback, feedback_reply='')
            staff_feedback.save()
            messages.success(request, 'Successfully Added For Feedback')
            return HttpResponseRedirect(reverse('staff_feedback'))
        except:
            messages.error(request, 'Failed To Add To Feedback')
            return HttpResponseRedirect(reverse('staff_feedback'))
...
def save_student_result(request):
    if request.method != 'POST':
        ...
        try:
            # check exist result with same subject and student id
            check_exist = StudentResult.objects.filter(student_id=student_obj, subject_id=subject_obj).exists()
            if check_exist:
                # update student result
                update_result = StudentResult.objects.get(student_id=student_obj, subject_id=subject_obj)
                update_result.subject_exam_marks = exam_marks
                update_result.subject_assignment_marks = assignment_marks
                update_result.save()
                messages.success(request, 'Successfully Updated Student Result')
                return HttpResponseRedirect(reverse('staff_add_result'))
            else:
                # add student result
                add_result = StudentResult(student_id=student_obj, subject_id=subject_obj, subject_exam_marks=exam_marks,subject_assignment_marks=assignment_marks)
                add_result.save()
...
def start_live_classroom_process(request):
    ...
    checks=OnlineClassRoom.objects.filter(subject=subject_obj,session_year=session_obj,is_active=True).exists()
    if checks:
        data=OnlineClassRoom.objects.get(subject=subject_obj,session_year=session_obj,is_active=True)
        ...
        staff_obj=Staffs.objects.get(admin=request.user.id)
        onlineClass=OnlineClassRoom(room_name=roomname,room_pwd=room_pwd,subject=subject_obj,session_year=session_obj,started_by=staff_obj,is_active=True)
        onlineClass.save()
...
def get_students(request):
    ...
    for student in students:
        st_data = {"id": student.admin.id, "name": student.admin.first_name +' '+student.admin.last_name}
        student_list.append(st_data)
...
def get_attendance_dates(request):
    ...
    subject_obj = Subjects.objects.get(id=subject_id)
    sesssion_obj = SessionYearModel.objects.get(id=session_year_id)

    attendance = Attendance.objects.filter(subject_id=subject_obj, session_year_id=sesssion_obj)
    attendance_list = []
    ...
    return JsonResponse(json.dumps(attendance_list), content_type="application/json", safe=False)
